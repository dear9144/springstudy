...
public class StaffServiceImpl implements StaffService {
	...
	public String addStaff1(HttpServletRequest request) {
		try {
			...
			staffMapper.addStaff(staffDTO); //예시) 사원번호가 5 BYTE 초과되거나 중복되거나 NULL인 경우 예외 발생해서 실패로 넘어가게 됨  
			return "사원등록이 성공했습니다";
		}catch(Exception e) {
			return "사원 등록이 실패했습니다"; //ajax의 error로 넘어감 
			}
		}
	...
	@Override
	public ResponseEntity<String> addStaff3(StaffDTO staffDTO) {
		try {
			staffMapper.addStaff(staffDTO);
			return new ResponseEntity<String>("사원등록이 성공햇습니다" , HttpStatus.OK);
		}catch(Exception e) {
			return new ResponseEntity<String>("사원번호는 5자리 숫자입니다", HttpStatus.BAD_REQUEST);
		}
	}
}
